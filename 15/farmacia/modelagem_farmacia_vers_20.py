...
NUM_SATELITES = 1
NUM_AUX_ALMOXARIFE = 2
NUM_AUX_POR_SATELITE = 2

random.seed()

# Tempo de trabalho destinado a cada atividade (minutos por dia)
TEMPO_TRABALHO = {
    "Conferência": 0.08*60,
    "Registro": 0.3*60,
    "Avaliação": 0.10*60,
    "Unit. Manual": 2.5*60,
    "Unit. Automática": 6*60,
    "Armazenamento CAF": 4*60,
    "Armaz. Interno": 3.5*60,
    "Ressuprimento": 11*60,
    "Armaz. Satélite": 4*60,
    "Atender Solic.": 0.01*60,
    "Produção Fita": 0.015*60,
    "Dispensação": 0.10*60,
}
...
def distribuicoes(tipo):
    return {
        ...
        "Etapa_producao_fita": random.betavariate(0.736, 1.233),
    ...
    def atualizar_stats(self, t_fila, uso_t, t_sistema, tempo_atual, count_items):
        ...
        try:
            if self.root.winfo_exists():
                ...
                # Calcular número de dias decorridos
                dias_decorridos = tempo_efetivo / (24 * 60)
                if dias_decorridos < 1/1440: dias_decorridos = 1/1440  # Mínimo de 1 minuto
                ...
                for nome in atividades_nomes:
                    ...
                    # Calcular uso percentual baseado no tempo de trabalho disponível
                    capacidade = 1
                    ...
                    # Tempo total disponível = dias * tempo_trabalho_diario * capacidade
                    tempo_disponivel = dias_decorridos * TEMPO_TRABALHO[nome] * capacidade
                    
                    percentual_uso = (uso_t[nome] / tempo_disponivel) * 100 if tempo_disponivel > 0 else 0
...
def Etapa_conferencia(env, gui, item, Aux_adm_of, Aux_adm_reg, Aux_far_frac, 
    ...
    with Aux_adm_of.request() as req:
        ...
        if env.now >= tempo_aquecimento:
            ...
            # Calcular uso baseado no tempo de trabalho disponível
            tempo_efetivo = env.now - tempo_aquecimento
            dias_decorridos = tempo_efetivo / (24 * 60)
            tempo_disponivel = dias_decorridos * TEMPO_TRABALHO["Conferência"]
            uso["Conferência"] = uso_t["Conferência"] / tempo_disponivel if tempo_disponivel > 0 else 0
...
def Etapa_registro(env, gui, item, Aux_adm_reg, Aux_almox_armz, Aux_far_frac, 
    ...
    with Aux_adm_reg.request() as req:
        ...
        if env.now >= tempo_aquecimento:
            ...
            tempo_efetivo = env.now - tempo_aquecimento
            dias_decorridos = tempo_efetivo / (24 * 60)
            tempo_disponivel = dias_decorridos * TEMPO_TRABALHO["Registro"]
            uso["Registro"] = uso_t["Registro"] / tempo_disponivel if tempo_disponivel > 0 else 0
        ...
        with Aux_almox_armz.request(priority=0) as req:
            try:
                ...
                if env.now >= tempo_aquecimento:
                    ...
                    tempo_efetivo = env.now - tempo_aquecimento
                    dias_decorridos = tempo_efetivo / (24 * 60)
                    tempo_disponivel = dias_decorridos * TEMPO_TRABALHO["Armazenamento CAF"]
                    uso["Armazenamento CAF"] = uso_t["Armazenamento CAF"] / tempo_disponivel if tempo_disponivel > 0 else 0
...
def Etapa_avaliar_fracio(env, gui, item, Aux_far_frac, Aux_far_unita_manual, unitali_auto, 
    ...
    with Aux_far_frac.request() as req:
        ...
        if env.now >= tempo_aquecimento:
            ...
            # Calcular uso baseado no tempo de trabalho disponível (2 recursos)
            tempo_efetivo = env.now - tempo_aquecimento
            dias_decorridos = tempo_efetivo / (24 * 60)
            tempo_disponivel = dias_decorridos * TEMPO_TRABALHO["Avaliação"] * 2
            uso["Avaliação"] = uso_t["Avaliação"] / tempo_disponivel if tempo_disponivel > 0 else 0
...
def Etapa_unita_manual(env, gui, item, Aux_far_unita_manual, Aux_almox_armz, Aux_far_ressupri, 
    ...
    with Aux_far_unita_manual.request() as req:
        ...
        if env.now >= tempo_aquecimento:
            ...
            tempo_efetivo = env.now - tempo_aquecimento
            dias_decorridos = tempo_efetivo / (24 * 60)
            tempo_disponivel = dias_decorridos * TEMPO_TRABALHO["Unit. Manual"] * NUM_AUX_ALMOXARIFE
            uso["Unit. Manual"] = uso_t["Unit. Manual"] / tempo_disponivel if tempo_disponivel > 0 else 0
...
def Etapa_unita_auto(env, gui, item, unitali_auto, Aux_almox_armz, Aux_far_ressupri, 
    ...
    with unitali_auto.request() as req:
        ...
        if env.now >= tempo_aquecimento:
            ...
            tempo_efetivo = env.now - tempo_aquecimento
            dias_decorridos = tempo_efetivo / (24 * 60)
            tempo_disponivel = dias_decorridos * TEMPO_TRABALHO["Unit. Automática"]
            uso["Unit. Automática"] = uso_t["Unit. Automática"] / tempo_disponivel if tempo_disponivel > 0 else 0
        ...
        with Aux_almox_armz.request(priority=1) as req:
            try:
                ...
                if env.now >= tempo_aquecimento:
                    ...
                    tempo_efetivo = env.now - tempo_aquecimento
                    dias_decorridos = tempo_efetivo / (24 * 60)
                    tempo_disponivel = dias_decorridos * TEMPO_TRABALHO["Armaz. Interno"]
                    uso["Armaz. Interno"] = uso_t["Armaz. Interno"] / tempo_disponivel if tempo_disponivel > 0 else 0
...
def Etapa_ressupri_satelite(env, gui, item, Aux_far_ressupri, Aux_far_satel, t_fila, uso_t, 
    ...
    with Aux_far_ressupri.request() as req:
        ...
        if env.now >= tempo_aquecimento:
            ...
            tempo_efetivo = env.now - tempo_aquecimento
            dias_decorridos = tempo_efetivo / (24 * 60)
            tempo_disponivel = dias_decorridos * TEMPO_TRABALHO["Ressuprimento"]
            uso["Ressuprimento"] = uso_t["Ressuprimento"] / tempo_disponivel if tempo_disponivel > 0 else 0
        ...
        with satelite_resource.request(priority=2) as req:
            try:
                ...
                if env.now >= tempo_aquecimento:
                    ...
                    tempo_efetivo = env.now - tempo_aquecimento
                    dias_decorridos = tempo_efetivo / (24 * 60)
                    tempo_disponivel = dias_decorridos * TEMPO_TRABALHO["Armaz. Satélite"] * NUM_SATELITES * NUM_AUX_POR_SATELITE
                    uso["Armaz. Satélite"] = uso_t["Armaz. Satélite"] / tempo_disponivel if tempo_disponivel > 0 else 0
        ...
        with satelite_resource.request(priority=1) as req:
            try:
                ...
                if env.now >= tempo_aquecimento:
                    ...
                    tempo_efetivo = env.now - tempo_aquecimento
                    dias_decorridos = tempo_efetivo / (24 * 60)
                    tempo_disponivel = dias_decorridos * TEMPO_TRABALHO["Atender Solic."] * NUM_SATELITES * NUM_AUX_POR_SATELITE
                    uso["Atender Solic."] = uso_t["Atender Solic."] / tempo_disponivel if tempo_disponivel > 0 else 0
        ...
        with satelite_resource.request(priority=3) as req:
            try:
                ...
                if env.now >= tempo_aquecimento:
                    ...
                    tempo_efetivo = env.now - tempo_aquecimento
                    dias_decorridos = tempo_efetivo / (24 * 60)
                    tempo_disponivel = dias_decorridos * TEMPO_TRABALHO["Produção Fita"] * NUM_SATELITES * NUM_AUX_POR_SATELITE
                    uso["Produção Fita"] = uso_t["Produção Fita"] / tempo_disponivel if tempo_disponivel > 0 else 0
...
def Etapa_dispensacao(env, gui, item, Aux_far_satel, t_fila, uso_t, uso, t_sistema, 
    ...
    while True:
        with satelite_resource.request(priority=0) as req:
            try:
                if gui and imprime_detalhes:
                    gui.log(f"{env.now:.2f}: Item {item.id} chegou em Dispensação — itens na fila: {len(satelite_resource.queue)}")
                yield req
                if env.now >= tempo_aquecimento:
                    t_fila["Dispensação"].append(env.now - chegada)
                    if gui and imprime_detalhes:
                        gui.log(f"{env.now:.2f}: Item {item.id} iniciou Dispensação")
                ini = env.now
                ...
                if env.now >= tempo_aquecimento:
                    ...
                    tempo_efetivo = env.now - tempo_aquecimento
                    dias_decorridos = tempo_efetivo / (24 * 60)
                    tempo_disponivel = dias_decorridos * TEMPO_TRABALHO["Dispensação"] * NUM_SATELITES * NUM_AUX_POR_SATELITE
                    uso["Dispensação"] = uso_t["Dispensação"] / tempo_disponivel if tempo_disponivel > 0 else 0
                    
                    if gui and imprime_detalhes:
                        gui.log(f"{env.now:.2f}: Item {item.id} finalizou Dispensação — tempo em fila={t_fila['Dispensação'][-1]:.2f} | uso_Total={uso_t['Dispensação']:.2f}")
...
def rodar_simulacao_unica(gui=None):
    ...
    Aux_almox_armz = simpy.PreemptiveResource(env, capacity=2)
    Aux_far_ressupri = simpy.Resource(env, capacity=2)
...
def imprimir_relatorio_final(resultados_replicacoes):
    ...
    for res in resultados_replicacoes:
        ...
        for nome in atividades_nomes:
            ...
            # Calcular uso percentual para esta replicação baseado no tempo de trabalho
            ...
            # Tempo disponível = dias * tempo_trabalho_diario * capacidade
            tempo_disponivel = dias_decorridos * TEMPO_TRABALHO[nome] * capacidade
            
            uso_perc = (res['uso_t'][nome] / tempo_disponivel) * 100 if tempo_disponivel > 0 else 0
    ...
    print(f"{'Atividade':<30} {'Uso Médio %':>15} {'IC(95%) Uso':>25} {'T_bar_Fila':>20} {'IC(95%) Fila':>20}")
    print("-" * 120)
    ...
    for nome in atividades_nomes:
        ...
        ic_uso = IC(uso_percentual_medio[nome]) if uso_percentual_medio[nome] else "(N/A)"
        ic_fila = IC(todos_tempos_fila[nome]) if todos_tempos_fila[nome] else "(N/A)"
        print(f"{nome:<30} {media_uso:>14.1f}% {ic_uso:>25} {media_fila:>20.2f} min {ic_fila:>20}")
