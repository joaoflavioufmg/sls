...
class Estatisticas():
    def __init__(self):
        ...
        self.occupancy_timeline = []  # Rastreamento de ocupação
    ...
    def averageQueueTime(self, activity, csv= False):
        if not self.events:
            return 0
        ...
        # Exporta Resultados
        if csv == True:
            waits.to_csv(f'tempos_fila({activity}).csv', index= False)
    ...
    def occupancyRate(self, resource, res_capacity, simTime, csv = False):  # ALTERADO
        """Calcula a taxa de ocupação de um recurso."""
        ...
        # Exporta csv
        if csv == True:
            occupancy.to_csv('tempos_ocupacao.csv', index= False)

        # Tempo total disponível = capacidade * tempo total da simulação
        total_available_time = res_capacity * simTime
    ...
    def track_occupancy(self, time, resource, res_capacity, interval=60):
        """Registra a taxa de ocupação em intervalos de tempo."""
        if len(self.occupancy_timeline) == 0 or time - self.occupancy_timeline[-1][0] >= interval:
            taxa = self.occupancyRate(resource, res_capacity, time)
            self.occupancy_timeline.append((time, taxa))
...
def triagem(env, data, entidade, recursos):
    ...
    with recursos['enfermeiro_triagem'].request() as req:  # não tem consultório, mas não precisa
...
def consulta_clinica(env, data, entidade, recursos):
    ...
    while True:
        # Se há exatamente dois 2 med_consulta e 2 consultórios, realmente modelar os consultórios?
        with recursos['med_consulta'].request(priority= entidade.prio) as req1, \
             recursos['consultorio'].request(priority= entidade.prio) as req2:  # Modificado para teste
            
            # registra evento (request para cada recurso)
            data.regEvent(env.now, 'ConsultaClinica', entidade, 'request', 'med_consulta')
            data.regEvent(env.now, 'ConsultaClinica', entidade, 'request', 'consultorio')  
            print(f"T = {env.now:.1f}: {entidade} entrou na fila da consulta clínica (pacientes na fila: {len(recursos['med_consulta'].queue)})")

            yield req1 & req2

            data.regEvent(env.now, 'ConsultaClinica', entidade, 'start', 'med_consulta')
        ...
        data.regEvent(env.now, 'ConsultaClinica', entidade, 'release', 'med_consulta')
        ...
        if desvio == 'A':
            ...
            if reavaliacoes >= 2:  # ALTERADO de == para >=
                print(f"T = {env.now:.1f}: {entidade} saiu do sistema")
                data.conta_saidas()
                data.regEvent(env.now, 'System', entidade, 'leave')
                return
...
def distributions(atividade):
    return {
        'chegada' : random.expovariate(lambd= 1 / 36), # aumentar a demanda em 50% = 24
...
def cria_recursos(env):
    recursos_dict = {
        ...
        'med_obstetra': Obstetra(env, capacidade=1),     # Capacidade modificada para teste
        'med_anestesista': simpy.PriorityResource(env, capacity=1),
        'med_pediatra': Pediatra(env, capacidade=3),
        'med_consulta' : simpy.PriorityResource(env, capacity= 2),   # Adicionado para teste
        ...
        # Estrutura Física
        'consultorio': simpy.PriorityResource(env, capacity=2),   # Não faz sentido 2 consultórios e 1 med_consulta
        'leito_inducao': simpy.PriorityResource(env, capacity=2), # ALTERADO
...
def track_obstetra_occupancy(env, data, recursos):
    """Processo que registra a taxa de ocupação do med_obstetra a cada intervalo."""
    interval = 60  # Intervalo de 60 minutos
    
    while True:
        yield env.timeout(interval)
        data.track_occupancy(env.now, 'med_consulta', 2, interval=interval)


def plot_occupancy_warmup(data, sim_time, intervention_time=None):
    """
    Plota a taxa de ocupação do med_obstetra ao longo da simulação.
    
    Args:
        data: Objeto Estatisticas com timeline de ocupação
        sim_time: Tempo total da simulação
        intervention_time: Tempo (opcional) para marcar início da coleta de dados
    """
    if not data.occupancy_timeline:
        print("Nenhum dado de ocupação disponível para plotagem.")
        return
    
    tempos = np.array([x[0] for x in data.occupancy_timeline])
    taxas = np.array([x[1] for x in data.occupancy_timeline])
    
    plt.figure(figsize=(14, 6))
    plt.plot(tempos, taxas, marker='o', linestyle='-', linewidth=2, markersize=4, color='#2E86AB')
    
    # Adiciona linha média
    media = taxas.mean()
    plt.axhline(y=media, color='red', linestyle='--', linewidth=2, label=f'Média: {media:.2f}%')
    
    # Marca período de warm-up (opcional)
    if intervention_time:
        plt.axvline(x=intervention_time, color='green', linestyle='--', linewidth=2, 
                   label=f'Fim Warm-up: {intervention_time:.0f} min')
        plt.fill_between(tempos, 0, 100, where=(tempos <= intervention_time), 
                        alpha=0.1, color='yellow', label='Período Warm-up')
    
    plt.xlabel('Tempo da Simulação (minutos)', fontsize=12)
    plt.ylabel('Taxa de Ocupação (%)', fontsize=12)
    plt.title('Ocupação do Médico Consulta ao Longo da Simulação', fontsize=14, fontweight='bold')
...
def run_replication(seed, sim_time):
    ...
    # Capacidades (para cálculo de ocupação)
    capacidades = {
        # Recursos Humanos
        'recepcionista': 1,
        'enfermeiro_triagem': 1,
        'med_obstetra': 3,
        'med_anestesista': 1,
        'med_pediatra': 3,
        ...
        # Equipe de Enfermagem
        'tec_enf_bloco': 6,   
        'enfermeiro_obstetra': 1,
        'enfermeiro_neonatal': 2,     
        'tec_enf_neonatal': 8,        
    ...
    results = {
        ...
        # Taxas de Ocupação (%)
       'Recepcionista (Cap=1)' : data.occupancyRate('recepcionista', 1, sim_time),
       'Enfermeiro Triagem (Cap=1)': data.occupancyRate('enfermeiro_triagem', 1, sim_time),
       'Consultório (Cap=2)': data.occupancyRate('consultorio', 2, sim_time),
       'Médico Obstetra (Cap=3)': data.occupancyRate('med_obstetra', 3, sim_time),
       'Médico Pediatra (Cap=3)': data.occupancyRate('med_pediatra', 3, sim_time),
       'Médico Anestesista (Cap=1)': data.occupancyRate('med_anestesista', 1, sim_time),
       'Sala Parto Normal (Cap=3)': data.occupancyRate('sala_parto_normal', 3, sim_time),
       'Sala Cesárea (Cap=1)': data.occupancyRate('sala_cesarea', 1, sim_time),
       'Sala Proc. Esp. (Cap=1)': data.occupancyRate('sala_proc_esp', 1, sim_time),
       'Leito de Indução (Cap=2)': data.occupancyRate('leito_inducao', 2, sim_time),
       'Alojamentos (Cap=25)': data.occupancyRate('alojamentos', 25, sim_time),
       'Leito UCP (Cap=16)': data.occupancyRate('leito_ucp', 16, sim_time),
       'Enfermeiro neonatal (Cap=2)': data.occupancyRate('enfermeiro_neonatal', 2, sim_time),
       'Técnico neonatal (Cap=8)': data.occupancyRate('tec_enf_neonatal', 8, sim_time),
       'Enfermeiro Obstetra (Cap=1)': data.occupancyRate('enfermeiro_obstetra', 1, sim_time),
       'Técnico Enf Bloco (Cap=6)': data.occupancyRate('tec_enf_bloco', 6, sim_time)
    }
...
def main():
    ...
    initial_seed = 3   # 1000 ; 3 ; 422937 ; 566755
...
SIM_TIME = 43800 # 30 dias em minutos
random.seed(0) # Semente para reprodutibilidade 42
...
# Rastreamento de ocupação do med_obstetra
env.process(track_obstetra_occupancy(env, data, recursos))

env.run(SIM_TIME)

# --- Resultados dos Contadores ---
print('\n--- Contadores Simples ---')
print(f"Chegaram ao sistema {data.contadorChegadas} paciente ({data.contadorUrgencias} urgências, {data.contadorChegadas - data.contadorUrgencias} eletivos)")
print(f"{data.contadorSaidas} pacientes saíram do sistema (pós-consulta ou reavaliação)")
print(f"{data.contadorInternacaoExt} pacientes foram internadas fora da Maternidade")
print(f"Foram realizadas {data.contadorPartosCesarea} cesáreas")
print(f"Foram realizados {data.contadorPartosNormal} partos normais")
print(f"Foram realizados {data.contadorProcEsp} Procedimentos Especiais")


# --- Resultados dos KPIs ---
print('\n--- KPIs Detalhados (em minutos) ---')

# Tempos de Fila
print(f"Tempo Médio Fila - Recepção: {data.averageQueueTime('Recepcao'):.2f} min")
print(f"Tempo Médio Fila - Triagem: {data.averageQueueTime('Triagem'):.2f} min")
print(f"Tempo Médio Fila - Consulta Clínica: {data.averageQueueTime('ConsultaClinica'):.2f} min")
print(f"Tempo Médio Fila - Leito Indução: {data.averageQueueTime('Inducao', csv= False):.2f} min")
print(f"Tempo Médio Fila - Sala Parto: {data.averageQueueTime('Parto', csv= False):.2f} min")

# Tempo de Sistema
print(f"\nTempo Médio de Sistema (Gestantes): {data.averageSystemTime():.2f} min")

# Taxas de Ocupação
print('\n--- Taxas de Ocupação ---')
print(f"Recepcionista (Cap=1): {data.occupancyRate('recepcionista', 1, SIM_TIME):.2f}%")
print(f"Enfermeiro Triagem (Cap=1): {data.occupancyRate('enfermeiro_triagem', 1, SIM_TIME):.2f}%")
print(f"Consultório (Cap=2): {data.occupancyRate('consultorio', 2, SIM_TIME):.2f}%")
print(f"Médico Obstetra (Cap=1): {data.occupancyRate('med_obstetra', 1, SIM_TIME):.2f}%")   # Capacidade Alterada para teste
print(f"Médico Consulta (Cap=2): {data.occupancyRate('med_consulta', 2, SIM_TIME):.2f}%")   # Adicionado para teste
print(f"Médico Pediatra (Cap=3): {data.occupancyRate('med_pediatra', 3, SIM_TIME):.2f}%")
print(f"Médico Anestesista (Cap=1): {data.occupancyRate('med_anestesista', 1, SIM_TIME):.2f}%")
print(f"Sala Parto Normal (Cap=3): {data.occupancyRate('sala_parto_normal', 3, SIM_TIME):.2f}%")
print(f"Sala Cesárea (Cap=1): {data.occupancyRate('sala_cesarea', 1, SIM_TIME):.2f}%")
print(f"Sala Proc. Esp. (Cap=1): {data.occupancyRate('sala_proc_esp', 1, SIM_TIME):.2f}%")
print(f"Leito de Indução (Cap=2): {data.occupancyRate('leito_inducao', 2, SIM_TIME):.2f}%")  # ALTERADO
print(f"Alojamentos (Cap=25): {data.occupancyRate('alojamentos', 25, SIM_TIME):.2f}%")
print(f"Leito UCP (Cap=16): {data.occupancyRate('leito_ucp', 16, SIM_TIME):.2f}%")
print(f"Enfermeiro neonatal (Cap=2): {data.occupancyRate('enfermeiro_neonatal', 2, SIM_TIME):.2f}%")
print(f"Técnico neonatal (Cap=8): {data.occupancyRate('tec_enf_neonatal', 8, SIM_TIME):.2f}%")
print(f"Enfermeiro Obstetra (Cap=1): {data.occupancyRate('enfermeiro_obstetra', 1, SIM_TIME):.2f}%")
print(f"Técnico Enf Bloco (Cap=6): {data.occupancyRate('tec_enf_bloco', 6, SIM_TIME):.2f}%")

# df = pd.DataFrame(data.events)
# df.to_csv('alta_demanda.csv', index= False)

# --- Plotagem da Taxa de Ocupação ---
print('\n--- Gerando Gráfico de Ocupação do Médico Obstetra ---')
# Define o tempo de warm-up (em minutos) - ajuste conforme necessário
warmup_time = 2880  # 2 dias = 48 horas = 2880 minutos
plot_occupancy_warmup(data, SIM_TIME, intervention_time=warmup_time)