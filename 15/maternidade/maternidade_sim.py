...
def distributions(atividade):
    return {
        'chegada' : random.expovariate(lambd= 1 / 24), # aumentar a demanda em 50% = 24 ; normal = 36
...
def cria_recursos(env):
    recursos_dict = {
        ...
        'med_obstetra': Obstetra(env, capacidade=2),     # Capacidade modificada para teste
        ...
        'tec_enf_bloco': simpy.PriorityResource(env, capacity=8),   # ALTERADO
        'enfermeiro_obstetra': simpy.PriorityResource(env, capacity=2), # ALTERADO
        ...
        'leito_inducao': simpy.PriorityResource(env, capacity=4), # ALTERADO

        # Salas específicas
        'sala_parto_normal': simpy.PriorityResource(env, capacity=4),  
        'sala_cesarea': simpy.PriorityResource(env, capacity=2),      # ALTERADO
...
def track_obstetra_occupancy(env, data, recursos):
    ...
    while True:
        ...
        data.track_occupancy(env.now, 'med_obstetra', 1, interval=interval)
...
def run_replication(seed, sim_time):
    ...
    results = {
        ...
        # 'fila_alojamento' : data.averageQueueTime('Alojamento'), # ADICIONADO

        # Tempo de Sistema (min)
        'tempo_sistema_gestantes': data.averageSystemTime(),

        # Taxas de Ocupação (%)
       'ocup_Recepcionista (Cap=1)' : data.occupancyRate('recepcionista', 1, sim_time),
       'ocup_Enfermeiro Triagem (Cap=1)': data.occupancyRate('enfermeiro_triagem', 1, sim_time),
       'ocup_Consultório (Cap=2)': data.occupancyRate('consultorio', 2, sim_time),
       'ocup_Médico Consulta (Cap=2)' : data.occupancyRate('med_consulta', 2, sim_time),  # ocup_
       'ocup_Médico Obstetra (Cap=1)': data.occupancyRate('med_obstetra', 1, sim_time),   # ocup_
       'ocup_Médico Pediatra (Cap=3)': data.occupancyRate('med_pediatra', 3, sim_time),
       'ocup_Médico Anestesista (Cap=1)': data.occupancyRate('med_anestesista', 1, sim_time),
       'ocup_Sala Parto Normal (Cap=3)': data.occupancyRate('sala_parto_normal', 3, sim_time),
       'ocup_Sala Cesárea (Cap=1)': data.occupancyRate('sala_cesarea', 1, sim_time),       # ALTERADO
       'ocup_Sala Proc. Esp. (Cap=1)': data.occupancyRate('sala_proc_esp', 1, sim_time),
       'ocup_Leito de Indução (Cap=2)': data.occupancyRate('leito_inducao', 2, sim_time),  # ALTERADO
       'ocup_Alojamentos (Cap=25)': data.occupancyRate('alojamentos', 25, sim_time),
       'ocup_Leito UCP (Cap=16)': data.occupancyRate('leito_ucp', 16, sim_time),
       'ocup_Enfermeiro neonatal (Cap=2)': data.occupancyRate('enfermeiro_neonatal', 2, sim_time),
       'ocup_Técnico neonatal (Cap=8)': data.occupancyRate('tec_enf_neonatal', 8, sim_time),
       'ocup_Enfermeiro Obstetra (Cap=1)': data.occupancyRate('enfermeiro_obstetra', 1, sim_time), # ALTERADO
       'ocup_Técnico Enf Bloco (Cap=6)': data.occupancyRate('tec_enf_bloco', 6, sim_time)
    }
...
def main():
    ...
    print(f'Seed Inicial: {initial_seed + 1}\n')
