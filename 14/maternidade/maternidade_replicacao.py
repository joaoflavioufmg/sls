...
class Estatisticas():
    def __init__(self):
        ...
        # Rastreamento de ocupação
        self.occupancy_timeline = []  # [(tempo, taxa_ocupacao), ...]
    ...
    def occupancyRate(self, resource, res_capacity, simTime):
...
def run_replication(seed, sim_time, track_occupancy_flag=False):
    ...
    # Processo para rastrear ocupação do med_obstetra a cada 60 minutos
    if track_occupancy_flag:
...
def track_obstetra_occupancy(env, data, recursos):
    ...
    while True:
        ...
        data.track_occupancy(env.now, 'med_obstetra', 3, interval=interval)
...
def plot_occupancy_warmup(data, sim_time, intervention_time=None):
    ...
    plt.title('Ocupação do Médico Obstetra ao Longo da Simulação', fontsize=14, fontweight='bold')
...
print(f"Tempo Médio Fila - Leito Indução: {data.averageQueueTime('Inducao'):.2f} min")
print(f"Tempo Médio Fila - Sala Parto: {data.averageQueueTime('Parto'):.2f} min")
...
print(f"Médico Obstetra (Cap=3): {data.occupancyRate('med_obstetra', 3, SIM_TIME):.2f}%")
...
print(f"Leito de Indução (Cap=2): {data.occupancyRate('leito_inducao', 2, SIM_TIME):.2f}%")
