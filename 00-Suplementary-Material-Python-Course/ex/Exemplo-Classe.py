# Listagem completa da nova agenda
########################################################################
...
class Menu:
    ...
    def execute(self):
        while True:
            self.exibe()
            escolha = valida_faixa_inteiro("Escolha uma opcao: ",
                         0, len(self.opcoes)-1)
            if escolha == 0:
                break
...
class AppAgenda:
    ...
    def apaga_telefone(self, dados):
        t = valida_faixa_inteiro_ou_branco(
            "Digite a poisção do numero a apagar, enter para sair: ",
    ...
    def altera_telefones(self,dados):
        t = valida_faixa_inteiro_ou_branco(
            "Digite a poisção do numero a alterar, enter para sair: ",
            0, len(dados.telefones)-1)
        if t == None:
            return
        telefone = dados.telefones[t]
        print("Telefone: %s" % telefone)
        print("Digite enter caso não queira alterar o numero")
